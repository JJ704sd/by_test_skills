...
Use map rather than slice when unresolved decisions determine the implementation route. Use `$elicit-stakeholder-input` live mode instead of map when only a bounded current-user judgment remains. Route raw external issues and pull requests to `$triage` regardless of how complete they appear.

Run exactly one mode per invocation. Charting or advancing a map, publishing a specification, and publishing slices are separate stopping points. Never advance automatically from one mode to another; require an explicit new request or confirmation after the current artifact is complete.
