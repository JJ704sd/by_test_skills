...
For each migration wave, pin a context capsule with baseline, exclusive paths and side effects, completion command and evidence, budget or stop condition, risks, and last checkpoint or resume gate. Parallelize read-only impact discovery or disjoint-path writes only within one unlocked current frontier when saved critical-path time exceeds dispatch, rereading, and fan-in cost. A single writer owns each shared boundary, generated artifact, and final integration. Old-path deletion, global preservation proof, and Git operations remain serial.

After every wave, create a preservation checkpoint. If the input, baseline, impact graph, or evidence changes, stop the current wave immediately, re-pin them, and revalidate affected work before resuming.
...
3. Run focused proof after every step. Inspect the diff for behavior edits, widened scope, generated noise, and unintended public-contract changes.
...
Report preserved invariants, baseline, structural transformations, evidence after each phase, final checks, and any behavior not covered by proof.
