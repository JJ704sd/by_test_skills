...
6. Run the smallest relevant formatting, type, build, and test checks. Inspect `git diff --check`, remaining unmerged paths, and the staged diff before continuing.
...
Subagents may analyze independent conflicts read-only from one pinned Git state only within an unlocked current frontier when saved critical-path time exceeds dispatch, rereading, and fan-in cost; pin the operation, commits, index stages, and hunks. A single resolver owns file writes, staging, and status checks, and continues only within the user's authorized merge goal. The user retains every abort decision; single-writer ownership is not authorization.

Resolve in waves, checkpointing unmerged paths, staged diff, and checks after each wave. Any Git-state change invalidates outstanding analysis.
