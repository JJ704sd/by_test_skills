...
handoff_packet:
  ...
  tool_execution_authorization: ALLOWED | BLOCKED | NOT_REQUIRED
  tool_authorization_ref: null
...
`snapshot_id` 绑定 build、范围、工具选型快照、门禁、用例、环境、配置、阈值和证据时间。依赖新选工具时，`tool_selection_snapshot_ref`、`tool_execution_authorization=ALLOWED` 和可复核的 `tool_authorization_ref` 都是必填项；缺失、失效或与当前工具版本不匹配时门禁必须为 `BLOCKED`。门禁、执行进度和生命周期是独立状态轴：`EXECUTION_COMPLETION` 使用 `machine_status=NOT_APPLICABLE`；测试准出必须是 `EXIT_GATE`。
