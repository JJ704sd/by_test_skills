...
`snapshot_id` 绑定能力、规模、团队标准、许可、环境、权限、安全制度和核验日期。采用结论不能推导出执行授权。`execution_authorization=ALLOWED` 时必须同时提供可复核的 `authorization_ref` 与 `approved_by`；本 skill 只能记录既有授权，不能把自己的选型建议当作授权来源。
