...
If `$triage` is available, recommend the canonical labels `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, and `wontfix`. Ask for overrides only when the repository already uses different labels, then write `docs/agents/triage-labels.md`.
